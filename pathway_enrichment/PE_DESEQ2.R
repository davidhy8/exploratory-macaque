...
full_data <- fread("all_age_DE.csv") 

# ID_conversion <- fread("genesymbol_entrezid_key.txt")

curr_gene_list = full_data$gene
...
file_name = 'KEGG_dot_all_age_DESEQ2.pdf'
pdf(file_name)
...
file_name = 'GO_dot_all_age_DESEQ2.pdf'
pdf(file_name)
print(dotplot(GO, showCategory=30))
dev.off()

file_name = 'GO_graph_all_age_DESEQ2.pdf'
pdf(file_name)
print(goplot(GO))
dev.off()

m_t2g <- msigdbr(species = "Macaca mulatta", category = "H") %>% 
